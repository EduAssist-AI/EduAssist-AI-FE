...
import VideoUpload from '../../components/CourseManagement/VideoUpload';
import { moduleApi, type ChatMessage as ModuleChatMessage, type Video, type VideoUploadResponse } from '../../api/modules';
...
export default function ModuleDetail() {
    ...
    try {
      ...
      if (response.status === 200) {
        if (Array.isArray(response.data)) {
          // If it returns an array, take the first element or find by ID
          const moduleData = response.data.find((m: any) => m.moduleId === moduleId);
  ...
  const fetchVideos = async () => {
    ...
    try {
      const response = await moduleApi.getModuleVideos(moduleId);
  ...
  const handleVideoUploadSuccess = (response: VideoUploadResponse) => {
    ...
    // Refresh the videos list to show the newly uploaded video
    fetchVideos();
  };
      ...
      case 'resources':
        return (
          <div className="p-6">
            ...
            <p className="text-gray-600 dark:text-gray-300">Resources for {module?.name}</p>
            <button 
              className="mt-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600"
              onClick={() => navigate(`/courses/${courseId}/modules/${moduleId}/resources`)}
            >
              View Resources
            </button>
          </div>
              ...
              <div className="p-4 flex-1 overflow-auto min-h-0">
                {showUploadForm ? (
                  ...
                  <>
                    ...
                    <div className="mb-6">
                      <h3 className="font-medium text-gray-800 dark:text-white mb-3">Module Videos</h3>
                            ...
                            >
                              <div className="flex items-center">
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="text-sm font-medium text-gray-800 dark:text-white truncate">{video.title}</p>
                                  <p className="text-xs text-gray-500 dark:text-gray-400">{Math.floor(video.durationSeconds / 60)}:{(video.durationSeconds % 60).toString().padStart(2, '0')}</p>
                                </div>
                              </div>
                            </div>
                          ))
                        ) : (
                          <p className="text-sm text-gray-500 dark:text-gray-400">No videos available for this module</p>
                        )}
                      </div>
                    </div>
                  </>
